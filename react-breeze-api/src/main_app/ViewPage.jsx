...
const ViewPage = () => {
    ...
    // const star_rate = retu.map((ret) =>{
    //     let value = 0;
    //     return value += ret.star_rating; 
    // })

    // const getRev = () => {
    //    const ret = reviews.filter((review) => {
    //         return review.business_name === formValues['business_name'];
    //     });
    //     setMatching(ret);
    // }
  ...
  return (
    <div>
        ...
        <div>
            <div className='w-full'>
                    {formValues['cover_image'] === '' || formValues['cover_image'] === null ? 
                        <div className='relative z-2'>
                            ...
                            :
                            <div className='relative z-2'>
                                <img src={"http://localhost:8000/" + formValues['cover_image']} alt={formValues['cover_image']} className="shadow-sm h-64 lg:h-96 w-full md:w-1/2 md:mx-auto object-cover"/>
                            </div>
                    }
            </div>
            <div className='p-2 px-3 space-y-2 md:space-y-2 lg:space-y-4 pt-4 md:w-1/2 md:mx-auto lg:mt-6'>
                <div className='flex flex-row justify-between px-1'>
                    <h5 className='text-lg sm:text-xl md:text-2xl text-gray-800 lg:text-4xl font-bold flex items-center gap-1 lg:gap-2'>{formValues['business_name']}
                        {
                        formValues['type'] == 'Authentic' ? 
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 text-green-600">
                    ...
                    </h5>
                    {
                        ...
                        <Link to={`/writeareview/${formValues['business_name']}/${id}`} className='hidden sm:flex items-center gap-1 px-3 p-1.5 bg-green-500 lg:gap-2 lg:px-3 lg:py-2 rounded-sm text-white hover:bg-green-400 shadow-lg hover:shadow-none transition-all ease-in-out duration-500'>
                            ...
                            <p className='text-sm sm:text-sm lg:text-base font-normal'>Write a review</p>
                        </Link>:
                ...
                <div className="items-center flex px-1">
                        {[...Array(5)].map((x, i) =>
                            <svg aria-hidden="true" className={Math.floor(star_rate / matchingReviews.length) > i ? "w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:h-7 lg:w-7 text-yellow-400" : "w-4 h-4 md:w-5 md:h-5 lg:h-7 lg:w-7 text-gray-300"} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" key={i}><title>Second star</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                        )}
                        {
                            matchingReviews.length < 2 ?
                            <p className="ml-2 text-xs md:text-sm lg:text-lg font-medium text-gray-500 ">{star_rate} out of 5</p>
                            : 
                            <p className="ml-2 text-xs md:text-sm lg:text-lg font-medium text-gray-500 ">{parseFloat(star_rate / matchingReviews.length).toFixed(1)} out of 5</p>
                        }
                </div>
                ...
                <div className='space-y-1 md:space-y-2 lg:space-y-4 py-2 px-1'>
                    <p className='text-sm md:text-sm lg:text-xl text-gray-800 font-bold mb-2'>About {formValues['business_name']}</p>
                    <div className='text-sm md:text-sm lg:text-base text-gray-500 pb-2'>
                ...
                <div className='space-y-3 md:space-y-2 lg:space-y-4 py-2 px-1'>
                    <p className='text-sm md:text-sm lg:text-xl text-gray-800 font-bold mb-2'>Location & Hours</p>
                    <div className=''>
                    ...
                    <div className='flex flex-col lg:flex-row lg:justify-between'>
                        <div className='space-y-2 md:space-y-2'>
                            <p className='text-sm sm:text-sm lg:text-base text-gray-500 flex text-start gap-1 lg:gap-3'> 
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-green-700 -ml-1">
                                        <path fillRule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" />
                                    </svg>
                                    <span><span className='font-medium text-gray-600'>{formValues['town']} - </span>{formValues['address']}</span>
                            </p>
                            <div className='flex gap-2 items-center text-gray-500 text-sm md:text-sm lg:text-base'>
                                    <p><span className='text-green-700 font-semibold'>Open</span> <span className='font-medium text-gray-600'>{formValues['open_from']}</span> - <span className='font-medium text-gray-600'>{[formValues['open_to']]}</span> from <span className='font-medium text-gray-600'>{[formValues['operating_from']]}</span> to <span className='font-medium text-gray-600'>{[formValues['operating_to']]}</span></p>
                            </div>

                        </div>
                        <div>
                            <a href={`https://maps.google.com/?q=${formValues['latitude']},${formValues['longitude']}`}>
                                <button className='h-fit w-full lg:w-fit mt-2 text-gray-700 bg-white border hover:bg-gray-200 border-gray-300 font-medium rounded-md text-sm lg:text-base px-3 p-1.5 lg:px-5 lg:py-2 text-center mr-2 active:scale-95 transition-all ease-out duration-500'>Get Directions</button>
                            </a>
                        </div>
                    </div>
                ...
                <hr />
                <div className='space-y-1 md:space-y-2 lg:space-y-4 py-2 px-1 '>
                    <p className='text-sm md:text-sm lg:text-xl text-gray-800 font-bold mb-2'>Amenities & More</p>
                ...
                <div className='space-y-2 md:space-y-2 lg:space-y-4 py-2 px-1'>
                    <p className='text-sm md:text-sm lg:text-xl text-gray-800 font-bold mb-2'>Contact Info</p>
                        <p className='text-sm md:text-sm lg:text-base text-gray-500 flex items-center gap-1 lg:gap-3'> 
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-6 lg:h-6 text-amber-500">
                                <path fillRule="evenodd" d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z" clipRule="evenodd" />
                            </svg>
                        {formValues['phone_number_one']} /   {formValues['phone_number_two']}
                        </p>
                        {/* <p className='text-[12px] sm:text-xs md:text-sm lg:text-base text-gray-500 flex items-center gap-1 lg:gap-3'> 
                        {formValues['phone_number_two']}
                        </p> */}
                    {
                        formValues['email'] != "" ? 
                        <p className='text-sm md:text-sm lg:text-base text-gray-500 flex items-center gap-2 lg:gap-3'> 
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-yellow-400">
                    ...
                    {
                        formValues['socials'] != "" ? 
                        <p className='text-sm md:text-sm lg:text-base text-gray-500 flex items-center gap-2 lg:grap-3'> 
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-blue-500">
            ...
            <div className='mt-4 md:w-1/2 md:mx-auto lg:mt-10 lg:pb-20' id='review-section'>
                <div className='flex gap-3 items-center px-4'>
                    <h5 className='text-[15px] sm:text-md lg:text-xl font-medium lg:font-bold flex items-center gap-1 lg:gap-2 mb-1'>Reviews</h5>
                    <p className='text-gray-500'>•</p>
                    ...
                    <div>
                        <Link to={`/writeareview/${formValues['business_name']}/${id}`} className='flex lg:hidden justify-between w-full items-center py-2 border md:border-0'>
                            <div className='flex px-3'>
                                {[...Array(5)].map((x, i) =>
                                    <svg aria-hidden="true" className="w-4 h-4 md:w-5 md:h-5 lg:w-7 lg:h-7 text-gray-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" key={i}><title>Second star</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                )}
                            </div>
                            <p className='text-sm lg:text-base md:text-sm text-gray-600 px-3'>Start your review</p>
                        </Link>
                    ...
                    <div id='reviews' className='px-3'>
                        {
                            reviews.filter(review => review.business_name === formValues['business_name']).map(reviewContent => (
                              ...
                              <p className='text-sm md:text-sm lg:text-xl text-center text-gray-600 font-medium mt-2 lg:mt-6 py-4'>No Reviews Yet</p>
                              :
                              <div className='text-[13px]  flex flex-col gap-2 mt-2 lg:gap-10 lg:mt-10'>
                              {reviews.filter(review => review.business_name === formValues['business_name']).map(reviewContent => (
                                    <div className='p-1' key={reviewContent.id}>
                                        <div className='px-1 space-y'>
                                            <div className='flex items-center gap-3'>
                                                ...
                                                <div>
                                                    <p className='text-[13px] md:text-sm lg:text-base font-semibold text-gray-800'>{reviewContent.author}</p>
                                                    <p className='text-[12px] md:text-xs lg:text-sm text-gray-500'>{reviewContent.created_at.split('T').shift()}</p>
                                                </div>
                                            </div>
                                            <div className='flex gap-3 items-center pt-4 lg:pt-6 mb-1'>
                                                <div className='flex'>
                                                        {[...Array(5)].map((x, i) =>
                                                            <svg aria-hidden="true" className={reviewContent.star_rating > i ? "w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:h-7 lg:w-7 text-yellow-400" : "w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:h-7 lg:w-7 text-gray-300"} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" key={i}><title>Second star</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                        )}
                                                </div>
                                                <p className='text-[12px] md:text-[13px] lg:text-sm text-gray-500'>{reviewContent.star_rating} out of 5</p>
                                            </div>
                                            <p className='text-gray-600 md:text-sm lg:text-base'>{reviewContent.comments}</p>
                                                <p></p>
                                            </div>
                                            <hr className='mt-3'/>
                                    </div>
