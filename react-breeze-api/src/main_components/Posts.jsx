              ...
              return(
                ...
                <div className='bg-white w-full flex p-4 sm:p-6 gap-3 sm:gap-6 justify-center border-b md:border border-gray-200 rounded-lg shadow  hover:shadow-lg hover:cursor-pointer transition-all ease-in-out duration-300 ' key={record.id}>
                  <div className='w-1/3'>
                  {
                    !record?.cover_image ?
                      <div>
                        <img src={defImage} alt="" className='object-cover rounded-md h-32 lg:h-fit w-full'/>
                      </div>
                      :
                      <div>
                        <img src={"http://localhost:8000/" + record.cover_image} alt="" className='object-cover rounded-md h-32 lg:h-fit w-full'/>
                      </div>
                  }
                  </div>
                                  
                  {/* mobile view */}
                          <div className='w-2/3 space-y-1 sm:space-y-1 lg:hidden'>
                                <h5 className='text-[14px] sm:text-md lg:text-base font-medium flex items-center gap-2 lg:gap-3'>{record.business_name}
                                {
                                ...
                                <div className="flex items-center">
                                    <svg aria-hidden="true" className={( reviews.filter(review => review.business_name === record.business_name).map(reviewContent => (
                                            ...
                                            )).length < 1) ? "w-5 h-5 sm:w-5 sm:h-5 text-gray-300" : "w-5 h-5 sm:w-5 sm:h-5 text-yellow-400"} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Rating star</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                    <span className="w-1 h-1 mx-1.5 bg-gray-500 rounded-full dark:bg-gray-400"></span>
                                      <p className="text-[12px] sm:text-xs md:text-sm text-gray-500 font-medium">
                                ...
                                <div className='text-blue-500 flex gap-1 items-center'>
                                      {
                                            record.specialties.split(',').map((specialties, index) => (
                                                <div className='flex items-center gap-2 px-1 text-gray-500 bg-gray-200 rounded-sm' key={index}>
                                                <span className='text-[11px] sm:text-xs font-medium'>{specialties}</span>
                                            </div> 
                                            ))
                                        
                                        }
                                          <p>•</p>
                                        <p className='text-[11px] sm:text-xs'>{record.category}</p>
                                </div>
                                {/* <p className='text-[11px] sm:text-xs md:text-sm text-gray-500 flex gap-1 items-center'>
                                  ...
                                  {record.phone_number_one} / {record?.phone_number_two}</p> */}
                                <div className='flex items-center gap-1'>
                                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-green-700 hidden sm:block">
                                        <path fillRule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" />
                                      </svg>
                                  <p className='text-[12px] md:text-sm sm:text-xs lg:text-sm text-gray-500'>
                                  <span className='font-medium text-gray-600'>{record.town}</span> - {record.address}</p>
                                </div>
                                <div className='flex gap-2 pt-1'>
                                     <div className='flex items-center gap-1'>
                                        {
                                          record.service_options.split(',').indexOf("Walk In") > -1 ? 
                                        ...
                                        <p className='text-[11px] md:text-sm'>Walk In</p>
                                     </div>
                                     <div className='flex items-center gap-1'>
                                        {
                                        ...
                                        <p className='text-[11px] md:text-sm'>Reservation</p>
                                     </div>
                                     <div className='flex items-center gap-1 '>
                                        {
                                        ...
                                        <p className='text-[11px] md:text-sm'>Delivery</p>
                                     </div>
                                </div>
                          ...
                          <div className='w-2/3 sm:space-y-2 hidden lg:block'>
                              <h5 className='text-sm sm:text-base lg:text-lg font-bold flex items-center gap-1 lg:gap-3'>{record.business_name}
                              {
                                record.type == 'Authentic' ? 
