...
const ArchivePage = () => {
    ...
    // const [isOpen, setModal] = useState(false);
        ...
        const { archives, getArchives, getTowns, towns, setErrors, errors, result, closeTownFast, townResult, closeResultFast, loading, setLoading, archiveResult, closeArchivedFast, openModal, closeModal, storeTown, onChangeTown, townFormValues} = useContext(RecordsContext);
        useEffect(() => {
            getArchives();
            getTowns();
            setLoading(true);
    ...
    const filterItems = (filterValue) => {;
        ...
        if (filterValue !== '') {
            const filteredData = archives.filter((archives) => {
                return Object.values(archives).join('').toLowerCase().includes(filterValue.toLowerCase())
                })
                setFilteredResults(filteredData);
                setIsFiltered(true);
        }
        else{
            setFilteredResults(archives)
            setIsFiltered(false);
          }
      }
    ...
    const currentPosts = archives.slice(indexOfFirstPost, indexOfLastPost);

    const paginate = (pageNumber) => setCurrentPage(pageNumber);

  return (
    <div className='min-h-screen max-h-full'>
        <TopHeading linkTo={'/dashboard/archived-records'} title={'Archives'} subtitle={'List'} id={''}/>
        ...
        <div className="w-4/5 mx-auto mt-12 h-fit">
            <div className='flex justify-between'>
                <h5 className='text-2xl font-semibold'>Archived Records</h5>
            ...
            <div className='mt-5'>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                    <div className="flex items-center rounded-xl bg-white shadow overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg p-6 space-y">
                        ...
                        <div className="ml-4">
                            <h2 className="font-semibold">Total Archives</h2>
                            <p className="mt-2 text-xl text-gray-500">{archives.length}</p>
                    ...
                    <div className="flex items-center rounded-xl bg-white shadow overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg p-6 space-y">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="mt-2 text-sm text-gray-500">{archives.length}</p>
                        </div>
                    </div>
          ...
          <div className="inline-block min-w-full py-2 align-middle mt-2">
                ...
                <Pagination postsPerPage={postsPerPage} totalPosts={isFiltered? filteredResults.length : archives.length} paginate={paginate} currentP={currentPage} loading={loading} indexOfFirstPost={indexOfFirstPost} indexOfLastPost={indexOfLastPost}/>
          </div>   
          <div id='deleteModal' className="hidden absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 modal w-1/4 ">
                    <div className="bg-white rounded-lg h-fit shadow">
                        ...
                        <form className="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" onSubmit={storeTown}>
                            ...
                            <button type="submit" className="w-full text-white bg-green-500 hover:bg-green-400 focus:bg-green-400 font-medium rounded-md text-sm px-5 py-2.5 text-center mr-2 mb-2 active:scale-95 transition-all ease-out ">Save Changes</button>
