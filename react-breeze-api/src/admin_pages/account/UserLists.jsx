...
const UserLists = () => {
        ...
        const {  getTowns, towns, setErrors, setLoading, loading } = useContext(RecordsContext);
        useEffect(() => {
            setLoading(true);
            getUsers();
            getTowns();
    ...
    const countUser = users.filter((user) => {
    ...
    const countVerified = users.filter((user) => {
        return user.role.toLowerCase() === 'user' && user.email_verified_at;
    }).length;

    const countUnverified = users.filter((user) => {
        return user.role.toLowerCase() === 'user' && !user.email_verified_at;
    }).length;
    ...
    const filterItems = (filterValue) => {;
        ...
        if (filterValue !== '') {
            const filteredData = guestUsers.filter((guestUsers) => {
                return Object.values(guestUsers).join('').toLowerCase().includes(filterValue.toLowerCase())
                })
                setFilteredResults(filteredData);
                setIsFiltered(true);
        }
        else{
            setFilteredResults(guestUsers)
            setIsFiltered(false);
          }
      }
    ...
    const currentPosts = guestUsers.slice(indexOfFirstPost, indexOfLastPost);

    const paginate = (pageNumber) => setCurrentPage(pageNumber);

  return (
    <div className='min-h-screen max-h-full'>
        <TopHeading linkTo={'/dashboard/users-records'} title={'Users'} subtitle={'List'} id={''}/>
        {result && (   
        ...
        <div className="w-4/5 mx-auto mt-12 h-fit">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                    <div className="flex items-center rounded-xl bg-white shadow overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg p-6 space-y">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="mt-2 text-xl text-gray-500">{countUser}</p>
                    ...
                    <div className="flex items-center rounded-xl bg-white shadow overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg p-6 space-y">
                        ...
                        <div className="ml-4">
                            <h2 className="font-semibold">Total Verified Users</h2>
                            <p className="mt-2 text-xl text-gray-500">{countVerified}</p>
                        </div>
                    </div>
                    <div className="flex items-center rounded-xl bg-white shadow overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg p-6 space-y">
                        ...
                        <div className="ml-4">
                            <h2 className="font-semibold">Total Unverified Users</h2>
                            <p className="mt-2 text-sm text-gray-500">{countUnverified}</p>
                        </div>
                    </div>
          ...
          <div className="inline-block min-w-full py-2 align-middle mt-2">
                ...
                <Pagination postsPerPage={postsPerPage} totalPosts={isFiltered? filteredResults.length : guestUsers.length} paginate={paginate} currentP={currentPage} loading={loading} indexOfFirstPost={indexOfFirstPost} indexOfLastPost={indexOfLastPost}/>
          </div>   
      </div>
