...
const TopHeading = ({ linkTo, title, subtitle, id}) => {
  const {user} = useAuthContext();


  return (
    <div>
        <div className="sticky z-10 top-0 h-16 border-b bg-white lg:py-2.5" id='modalId'>
            <div className="px-6 flex items-center justify-between space-x-4 2xl:container">
                {
                    id == "" ?            
                    <p className='flex gap-4 align-center text-gray-500'> <Link to={linkTo}><span hidden className="text-md text-gray-700 font-medium lg:block">{title}</span></Link> <span className='h-6 border-r border-gray-300 -skew-x-12 '></span> {subtitle} </p>
                    :
                    <p className='flex gap-4 align-center text-gray-500'> <Link to={linkTo}><span hidden className="text-md text-gray-700 font-medium lg:block">{title}</span></Link> <span className='h-6 border-r border-gray-300 -skew-x-12 '></span> {subtitle} <span className='h-6 border-r border-gray-300 -skew-x-12 '></span> {id}</p>
